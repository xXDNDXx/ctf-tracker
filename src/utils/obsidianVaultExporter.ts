...
> **Operator Callsign:** \`${operatorCallsign}\`  
...
`;
    ...
    if (m.credentials && m.credentials.length > 0) {
      doc += `\n---\n\n## 🔑 Compromised Credentials & Loot Vault\n\n`;
      doc += `| Identity | Secret | Type | Target Service | Realm / Domain | Privileged |\n`;
      doc += `| :--- | :--- | :--- | :--- | :--- | :--- |\n`;
      const cleanCol = (str?: string) => (str || '').replace(/\|/g, '\\|').replace(/[\r\n]+/g, ' ').trim();
      m.credentials.forEach((c) => {
        const u = cleanCol(c.username);
        const s = cleanCol(c.secret);
        const d = cleanCol(c.domain) || 'Local';
        const srv = cleanCol(c.service).toUpperCase();
        doc += `| \`${u}\` | \`${s}\` | ${c.type.toUpperCase()} | ${srv} | ${d} | ${c.isPrivileged ? '🛡️ YES' : 'NO'} |\n`;
      });
      doc += `\n`;
    }
