...
**Candidate Name:** ${session.candidateName || 'Candidate Operator'}  
**Callsign / Handle:** ${session.candidateCallsign || 'Operator'}  
**OSID:** ${session.osid || 'OS-XXXXX'}  
...
`;
  ...
  const isOscapTrack = session.track === 'OSCP';
  const adBoxes = session.boxes.filter((b) => b.type.startsWith('ad-'));
  const allAdPwned = !isOscapTrack || adBoxes.every((b) => (b.userPoints === 0 || b.userPwned) && (b.rootPoints === 0 || b.rootPwned));

  session.boxes.forEach((b) => {
    const isAdBox = isOscapTrack && b.type.startsWith('ad-');
    let pts = (b.userPwned ? b.userPoints : 0) + (b.rootPwned ? b.rootPoints : 0);
    let adNote = '';
    if (isAdBox && !allAdPwned && pts > 0) {
      adNote = ' *(Chain Incomplete: 0 PTS)*';
      pts = 0;
    }
    ...
    report += `| **${b.name}** | ${b.label} | \`${b.ip}\` | ${userStatus} | ${rootStatus} | **${pts} PTS**${adNote} |
`;
  });
...
`;
  ...
  report += `---
*Generated by ZeroBox Tactical CTF Platform // Candidate: ${session.candidateName || 'Operator'} (${session.candidateCallsign || 'Operator'})*
`;
