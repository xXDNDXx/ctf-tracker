...
export function evaluateRabbitHoleStatus(
  ...
  const startedAtMs = activeRecord?.startedAt ? new Date(activeRecord.startedAt).getTime() : 0;
  const liveElapsedSeconds = (startedAtMs > 0 && !activeRecord?.completedAt)
    ? Math.floor((Date.now() - startedAtMs) / 1000)
    : 0;
  const timeSpentSeconds = (activeRecord?.timeSpentSeconds || 0) + liveElapsedSeconds;
