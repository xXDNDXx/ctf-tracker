...
export const ExamSimulatorPage: React.FC = () => {
  ...
  const toggleTimer = () => {
    if (session.isTimerRunning) {
      ...
      const duration = (typeof session.timerPausedRemainingSeconds === 'number' && session.timerPausedRemainingSeconds > 0)
  ...
  // Reset Timer to default track duration
  const resetTimer = () => {
    const defaultDuration = session.track === 'CPTS' ? 48 * 3600 : 24 * 3600;
    setSession((prev) => ({
      ...prev,
  ...
  return (
    <div className="w-full space-y-5 font-mono">
      ...
      <div className="p-4 rounded-xl border border-slate-200 dark:border-cyber-border bg-white dark:bg-cyber-card/90 shadow-xl space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-2 border-t border-slate-100 dark:border-cyber-border/40 text-xs">
          <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-slate-50 dark:bg-cyber-bg border border-slate-200 dark:border-cyber-border/70">
            <span className="text-[10px] text-slate-500 dark:text-cyber-muted uppercase font-bold tracking-wider">Candidate:</span>
            <input
              id="exam-candidate-name"
              name="exam-candidate-name"
              type="text"
              value={session.candidateName || ''}
              onChange={(e) => setSession((prev) => ({ ...prev, candidateName: e.target.value }))}
              placeholder="Candidate Name..."
              className="bg-transparent text-xs text-slate-900 dark:text-white font-mono focus:outline-none flex-1 placeholder-slate-400 dark:placeholder-cyber-muted min-w-0"
            />
          </div>
          <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-slate-50 dark:bg-cyber-bg border border-slate-200 dark:border-cyber-border/70">
            <span className="text-[10px] text-slate-500 dark:text-cyber-muted uppercase font-bold tracking-wider">Callsign:</span>
            <input
              id="exam-candidate-callsign"
              name="exam-candidate-callsign"
              type="text"
              value={session.candidateCallsign || ''}
              onChange={(e) => setSession((prev) => ({ ...prev, candidateCallsign: e.target.value }))}
              placeholder="Callsign / Handle..."
              className="bg-transparent text-xs text-slate-900 dark:text-white font-mono focus:outline-none flex-1 placeholder-slate-400 dark:placeholder-cyber-muted min-w-0"
            />
          </div>
          <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-slate-50 dark:bg-cyber-bg border border-slate-200 dark:border-cyber-border/70">
            <span className="text-[10px] text-slate-500 dark:text-cyber-muted uppercase font-bold tracking-wider">OSID:</span>
            <input
              id="exam-candidate-osid"
              name="exam-candidate-osid"
              type="text"
              value={session.osid || ''}
              onChange={(e) => setSession((prev) => ({ ...prev, osid: e.target.value }))}
        ...
        {scoreData.complianceIssues.length > 0 && (
