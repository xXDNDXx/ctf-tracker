...
export const WarRoomView: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] min-h-[600px] bg-[#050811] text-slate-200 font-mono overflow-hidden rounded-xl border border-cyber-border shadow-2xl">
      {/* Top Operations Deck Status Bar */}
      <div className="flex-shrink-0 bg-[#080d19] border-b border-cyber-border px-4 py-2.5 flex flex-wrap items-center justify-between gap-3 text-xs">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-black tracking-widest text-white uppercase flex items-center gap-1.5">
              WAR ROOM <span className="text-cyber-cyan">//</span> DUAL-PANE OPS DECK
            </span>
          </div>

          <span className="hidden sm:inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-cyan-500/10 text-cyber-cyan border border-cyan-500/30 uppercase tracking-wider">
            OFFENSIVE OPS MODE
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="warroom-target-select" className="text-[11px] text-cyber-muted font-bold uppercase hidden md:inline">
            Active Target:
          </label>
          <select
            ...
            className="bg-[#03060d] text-xs font-bold text-cyber-cyan px-2.5 py-1 rounded border border-cyber-border focus:border-cyber-cyan outline-none max-w-[180px] sm:max-w-[240px] truncate"
          >
            {machines.map((m) => (
          ...
          <button
            ...
            className="px-2 py-1 rounded bg-cyber-card hover:bg-cyber-cyan/15 text-slate-300 hover:text-cyber-cyan border border-cyber-border hover:border-cyber-cyan/40 transition-all text-xs font-bold flex items-center gap-1"
          >
            <Dices className="w-3.5 h-3.5 text-cyber-cyan" />
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-2 py-1 rounded bg-cyan-500/10 hover:bg-cyan-500/25 text-cyber-cyan border border-cyan-500/30 text-[11px] font-bold flex items-center gap-1 transition-all"
          >
          ...
          <button
            ...
            className="px-2 py-1 rounded bg-purple-500/10 hover:bg-purple-500/25 text-purple-400 border border-purple-500/30 text-[11px] font-bold flex items-center gap-1 transition-all"
          >
          ...
          <button
            ...
            className="px-2 py-1 rounded bg-amber-500/10 hover:bg-amber-500/25 text-amber-400 border border-amber-500/30 text-[11px] font-bold flex items-center gap-1 transition-all"
          >
          ...
          <div className="hidden lg:flex items-center gap-1 bg-[#03060d] p-0.5 rounded border border-cyber-border text-[10px]">
            {[
          ...
          <button
            ...
            className="p-1.5 rounded bg-cyber-card hover:bg-slate-800 text-slate-300 hover:text-white border border-cyber-border transition-all"
          >
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div
          className={`flex flex-col border-b lg:border-b-0 lg:border-r border-cyber-border bg-[#03060d] relative overflow-hidden transition-all duration-200 ${
            splitRatio === '70-30'
        ...
        >
          ...
          <div className="absolute top-3 left-3 z-20 flex items-center gap-2 bg-[#080d19]/90 backdrop-blur-md px-3 py-1.5 rounded-lg border border-cyber-border text-xs">
            <Radio className="w-3.5 h-3.5 text-cyber-cyan animate-pulse" />
            <span className="font-bold text-white tracking-wider">LIVE TOPOLOGY CLUSTER</span>
            <span className="text-[10px] text-cyber-muted font-mono">({machines.length} nodes)</span>
          </div>
        ...
        <div
          className={`flex flex-col bg-[#070b16] overflow-hidden transition-all duration-200 ${
            splitRatio === '70-30'
        ...
        >
          ...
          {activeTarget && (
            <div className="flex-shrink-0 bg-[#0a0f1d] border-b border-cyber-border p-3.5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-2 bg-[#050811] px-2 py-1 rounded border border-cyber-border">
                    ...
                    <div className="flex items-center gap-0.5 border-l border-cyber-border pl-1.5 ml-1">
                      ...
                      <button
                        ...
                        className="p-1 rounded hover:bg-slate-800 text-cyber-muted hover:text-white transition-colors"
                      >
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 rounded text-xs font-bold transition-all border flex items-center gap-1 ${
                      activeTarget.userPwnedAt
                        ...
                        : 'bg-cyber-card text-slate-400 hover:text-white border-cyber-border'
                    }`}
                  >
              ...
              {activeTarget.openPorts && activeTarget.openPorts.length > 0 && (
                <div className="flex items-center gap-1.5 mt-2.5 overflow-x-auto text-[10px]">
                  <span className="text-cyber-muted font-bold flex-shrink-0">PORTS:</span>
                  {activeTarget.openPorts.map((p) => (
                    <span
                      key={p}
                      className="px-1.5 py-0.5 rounded bg-slate-900 border border-cyber-border text-emerald-400 font-bold"
                    >
                      {p}
                    </span>
                  ))}
                  {activeTarget.cves && activeTarget.cves.length > 0 && (
                    <>
                      <span className="text-cyber-muted font-bold ml-2 flex-shrink-0">CVES:</span>
                      {activeTarget.cves.map((cve) => (
                        <span
                          key={cve}
                          className="px-1.5 py-0.5 rounded bg-rose-500/10 border border-rose-500/30 text-rose-400 font-bold"
                        >
                          {cve}
                        </span>
                      ))}
          ...
          <div className="flex-shrink-0 bg-[#080d19] border-b border-cyber-border px-3 flex items-center gap-1 text-xs">
            {[
              ...
              >
                ...
                {t.count !== undefined && t.count > 0 && (
                  <span className="px-1.5 py-0.2 rounded-full text-[9px] bg-cyber-cyan/20 text-cyber-cyan font-extrabold">
                    {t.count}
                  </span>
                )}
              </button>
          ...
          {/* Operations Tab Body */}
          <div className="flex-1 overflow-y-auto p-4 bg-[#060a14]">
              ...
              <div className="flex flex-col gap-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-[#03060d] text-xs text-white pl-9 pr-3 py-2 rounded-lg border border-cyber-border focus:border-cyber-cyan outline-none placeholder:text-slate-600"
                  />
                </div>
                ...
                <div className="space-y-2">
                  {tacticalCommands.map((cmd, idx) => {
                    ...
                    return (
                      <div
                        key={cmd.id || idx}
                        className="p-2.5 rounded-lg border border-cyber-border bg-[#0a0f1d] hover:border-cyber-cyan/40 transition-all group"
                      >
                        ...
                        <pre className="p-2 rounded bg-[#03060d] border border-cyber-border text-xs text-emerald-400 font-mono overflow-x-auto select-all">
                          {interpolated}
                        </pre>
                      </div>
                    );
            ...
            {activeTab === 'notes' && activeTarget && (
              <div className="flex flex-col h-full gap-2">
                <div className="flex items-center justify-between">
                  <label htmlFor="warroom-field-notes" className="text-xs font-bold text-cyber-cyan uppercase flex items-center gap-1">
                ...
                <textarea
                  ...
                  className="w-full flex-1 bg-[#03060d] text-slate-200 font-mono text-xs sm:text-sm p-3.5 rounded-lg border border-cyber-border focus:border-cyber-cyan focus:ring-1 focus:ring-cyber-cyan outline-none resize-none leading-relaxed placeholder:text-slate-600"
                />
              </div>
            )}
            ...
            {activeTab === 'forge' && (
              <div className="flex flex-col gap-3">
                <div>
                  <label htmlFor="warroom-quick-payload" className="text-xs font-bold text-cyber-cyan uppercase flex items-center gap-1 mb-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-[#03060d] text-emerald-400 font-mono text-xs p-2.5 rounded-lg border border-cyber-border focus:border-cyber-cyan outline-none"
                  />
                </div>
                ...
                <div className="space-y-2">
                  {forgeResults.map((item) => (
                    <div
                      key={item.id}
                      className="p-2 rounded-lg border border-cyber-border bg-[#0a0f1d] flex flex-col gap-1"
                    >
                      <div className="flex items-center justify-between">
                        ...
                        >
                          {copiedForgeId === item.id ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
                          {copiedForgeId === item.id ? 'Copied' : 'Copy'}
                        </button>
                      </div>
                      <pre className="p-1.5 rounded bg-[#03060d] border border-cyber-border text-xs text-emerald-400 font-mono overflow-x-auto whitespace-pre-wrap break-all select-all">
                        {item.output}
                      </pre>
                    </div>
                  ))}
