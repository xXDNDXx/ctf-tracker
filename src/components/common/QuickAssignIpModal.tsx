...
export const QuickAssignIpModal: React.FC = () => {
  ...
  const isOpen = Boolean(assignIpMachineId && machine);

  useModalA11y({
    isOpen,
    onClose: () => setAssignIpMachineId(null),
  ...
  const handleConfirm = () => {
    ...
    const IP_HOSTNAME_REGEX = /^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$|^[a-fA-F0-9:]+$|^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/;
    if (!IP_HOSTNAME_REGEX.test(cleanIp)) {
      setErrorMsg('Invalid IP address or target hostname format.');
      return;
    }
  ...
  const content = (
    ...
    >
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="quick-assign-ip-title"
