...
import { interpolateCommand, playCyberSound, triggerRootCelebration, safeCopyToClipboard } from '../../utils/helpers';
...
export const ChecklistWorkspace: React.FC<ChecklistWorkspaceProps> = ({
  ...
  // Handle ESC key and body scroll lock for Nmap modal
  useEffect(() => {
    if (!nmapModalOpen) return;
  ...
  const handleApplyNmapScan = () => {
    ...
    // 1. Try universal scan parser first (handles XML, Rustscan, Grepable, standard Nmap)
    const scanResult = detectAndParseScan(nmapInputText);
    let extractedPorts: number[] = [];

    if (scanResult && scanResult.ports.length > 0) {
      extractedPorts = scanResult.ports.map((p) => p.port);
    } else {
      // 2. Fallback to classic Nmap parser
      const { ports } = parseNmapScanOutput(nmapInputText);
      extractedPorts = ports;
    }

    if (extractedPorts.length > 0) {
      const merged = Array.from(new Set([...openPorts, ...extractedPorts])).sort((a, b) => a - b);
      ...
      if (scanResult) {
        ...
        const scanNotes = `\n\n### ⚡ Quick Nmap Scan Intake (${timestamp})\n` +
          scanResult.ports.map((p) => `- **Port ${p.port}/${p.protocol}**: ${p.service.toUpperCase()} ${p.version ? `(${p.version})` : ''}`).join('\n');

        updateMachine(machine.id, {
          openPorts: merged,
          writeupMarkdown: updatedWriteup,
          quickNotes: (machine.quickNotes || '') + scanNotes,
        });
      } else {
        updateMachine(machine.id, {
          openPorts: merged,
      ...
      setNmapError('No open ports detected in the pasted text. Ensure the output contains open port lines (e.g. "80/tcp open http") or use the Full Recon Hub.');
      if (soundEnabled) playCyberSound('toggle');
    }
  };
  ...
  const handleCopySnippet = async (rawSnippet: string, itemId: string) => {
  ...
  return (
    <div className="space-y-4 font-mono text-xs text-cyber-text">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <button
              ...
              title="Quick paste raw Nmap stdout"
            >
            ...
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1 rounded bg-cyan-950/40 border border-cyan-500/40 text-cyan-300 hover:bg-cyber-cyan hover:text-black text-xs font-semibold transition-all shadow-glow-cyan/20"
              title="Open full Recon Automation Hub (XML, Rustscan, Grepable & Payloads)"
            >
              <Zap className="w-3.5 h-3.5" />
              <span>Full Recon Hub</span>
            </button>
          </div>
        </div>
      ...
      {nmapModalOpen && typeof document !== 'undefined' && createPortal(
        <div 
          className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in font-mono"
          onClick={() => {
            setNmapModalOpen(false);
            setNmapError(null);
          }}
        >
          <div 
            role="dialog"
            aria-modal="true"
            aria-labelledby="nmap-ingest-title"
            className="w-full max-w-xl rounded-xl border border-cyber-border bg-cyber-card shadow-2xl p-5 space-y-4"
          ...
          >
            <div className="flex items-center justify-between border-b border-cyber-border pb-2.5">
              <div className="flex items-center gap-2">
                ...
                <h3 id="nmap-ingest-title" className="font-bold text-white text-sm tracking-wide">QUICK NMAP / SCAN INGESTION</h3>
              </div>
              <button
                onClick={() => {
                  setNmapModalOpen(false);
                ...
                aria-label="Close scan ingestion modal"
            ...
            <p className="text-xs text-cyber-muted">
              Paste the stdout of your scan (e.g. <code className="text-white">nmap -sC -sV</code>, Nmap XML, Grepable, or Rustscan). Open ports will be automatically extracted and linked to dynamic methodology attack trees!
            </p>
            ...
            {nmapError && (
              <div className="p-3 rounded-lg bg-rose-950/40 border border-rose-500/50 text-rose-300 text-xs flex items-start gap-2">
                ...
                <div className="space-y-1">
                  <span className="font-bold">Scan Ingestion Notice</span>
                  <p className="text-[11px] leading-relaxed">{nmapError}</p>
                </div>
              </div>
            )}
            ...
            <textarea
              ...
              value={nmapInputText}
              onChange={(e) => {
                setNmapInputText(e.target.value);
                if (nmapError) setNmapError(null);
              }}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2 border-t border-cyber-border">
              <button
                type="button"
                onClick={() => {
                  setNmapModalOpen(false);
                ...
                className="text-xs text-cyber-cyan hover:underline flex items-center gap-1.5 font-mono font-semibold"
              >
                <Zap className="w-3.5 h-3.5" />
                <span>Switch to Full Recon Automation Hub</span>
              </button>
              ...
              <div className="flex items-center gap-2 self-end sm:self-auto">
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-cyber-bg border border-cyber-border text-cyber-muted hover:text-white text-xs transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-cyber-cyan text-black font-bold text-xs hover:bg-cyan-300 transition-all shadow-glow-cyan disabled:opacity-50"
                >
