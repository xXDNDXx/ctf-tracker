...
export const Sidebar: React.FC = () => {
  ...
  const navItems = [
    ...
    },
    {
      id: 'cheatsheet',
      path: '/cheatsheets',
      label: 'Tactical & Field Manual',
      icon: Terminal,
      sublabel: `${cheatsheets.length} Snippets • ${userNotes.length > 0 ? `${userNotes.length} Vault Notes` : 'Obsidian Vault'}`,
      badge: `${cheatsheets.length}`,
  ...
  const handleViewModeSelect = (mode: typeof viewMode) => {
    setViewMode(mode);
  ...
  return (
    <aside
      className={`hidden md:flex relative flex-col h-full overflow-hidden border-r border-slate-200 dark:border-cyber-border bg-white/95 dark:bg-cyber-bg text-slate-900 dark:text-cyber-text transition-all duration-300 z-30 ${
        collapsed ? 'w-16' : 'w-72'
      }`}
    >
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin px-2.5 py-4 space-y-1.5 font-mono">
        ...
        {activeTab === 'tracker' && !collapsed && (
          <div className="mt-6 pt-4 border-t border-cyber-border/70">
            ...
            <div className="grid grid-cols-4 gap-1 bg-cyber-card p-1 rounded-lg border border-cyber-border">
              <button
                onClick={() => handleViewModeSelect('kanban')}
              ...
              <button
                onClick={() => handleViewModeSelect('graph')}
                className={`flex flex-col items-center justify-center py-1.5 rounded text-[10px] transition-all ${
                  viewMode === 'graph'
                    ? 'bg-cyber-emerald/20 text-cyber-emerald border border-cyber-emerald/40 font-bold'
                    : 'text-cyber-muted hover:text-slate-900 dark:hover:text-white'
      ...
      {!collapsed ? (
        <div className="p-3 border-t border-slate-200 dark:border-cyber-border bg-slate-50/90 dark:bg-cyber-card/40 font-mono text-xs flex-shrink-0">
