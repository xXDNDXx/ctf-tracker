...
export const MobileNav: React.FC = () => {
  ...
  return (
    <>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-cyber-bg/95 backdrop-blur-lg border-t border-cyber-border/80 px-2 pt-1.5 pb-[calc(0.375rem+env(safe-area-inset-bottom))] font-mono shadow-[0_-5px_20px_rgba(0,0,0,0.5)]">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs scrollbar-thin">
                ...
                <div className="space-y-1">
                  ...
                  <button
                    onClick={() => handleNavClick('/cheatsheets', 'cheatsheet')}
                    className={`w-full p-2.5 rounded-lg border flex items-center justify-between text-left transition-colors ${
                      location.pathname.startsWith('/cheatsheet') || location.pathname.startsWith('/field-manual') || location.pathname.startsWith('/notes') || location.pathname.startsWith('/cpts')
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-purple-950/40 text-purple-300 border border-purple-800/40 font-mono">
                        {userNotes.length > 0 ? `${cheatsheets.length}+${userNotes.length}` : `${cheatsheets.length}`}
                      </span>
